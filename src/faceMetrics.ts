import { canthalTilt, JAW, type Landmark } from "./geometry";
import {
	faceBoxFromPoints,
	headPose,
	PITCH_MAX,
...
// Single-metric measurements for the on-device tool pages (face shape,
// symmetry, canthal tilt, …). Pure, total, never throws: every entry point
// returns a gate + a value (null when the gate refuses or the mesh is junk).
//
// Conventions, same as faceGeometry.ts / photoQuality.ts:
// - landmarks are MediaPipe's 478-point mesh, normalized 0–1 in image space;
// - "left"/"right" are the SUBJECT's side, so the subject's right eye (33)
//   sits on the image's left;
// - all measurements happen in pixel-consistent space, px(p) = {x·aspect, y},
//   after removing head roll by rotating about the midpoint of the forehead
//   top (10) and the menton (152) until that line is vertical.
...
export const TOOL_METRICS: readonly ToolMetric[] = [
	"face-shape",
	"face-symmetry",
	"golden-ratio",
	"canthal-tilt",
...
export type Gate =
	| { ok: true; confidence: "high" | "medium" }
	| { ok: false; reason: "yaw" | "pitch" | "roll" | "small" | "points" };

// Landmark indices, drawn by ToolOverlay over the ORIGINAL (unrotated) mesh.
// hlines/vlines are horizontal/vertical guides through that landmark.
export type Overlay = {
	points?: number[];
	lines?: [number, number][];
	polylines?: number[][];
...
export type MetricResult<T> = { gate: Gate; value: T | null; overlay: Overlay };
...
export type FaceShapeValue = {
	shape: FaceShapeName;
	// 1 − d(best)/d(runner-up), 0–1: how clearly the best prototype wins.
	match: number;
	runnerUp: FaceShapeName;
	ratios: { length: number; forehead: number; jaw: number; chin: number };
	gonial: number;
};

export type SymmetryValue = {
	score: number;
	parts: {
		eyes: number;
		brows: number;
...
export type GoldenRatioValue = {
	score: number;
	ratios: { name: string; value: number; target: number; score: number }[];
};

export type CanthalTiltValue = { left: number; right: number; mean: number };

export type HunterEyesValue = {
	score: number;
	ar: number;
	browGap: number;
	tilt: number;
...
export type FacialRatiosValue = {
	thirds: [number, number, number];
	fifths: [number, number, number, number, number];
	fwhr: number;
	midface: number;
};

export type EyeShapeValue = {
	shape: "almond" | "round" | "narrow";
	modifier: "upturned" | "downturned" | "level";
	ar: number;
	tilt: number;
	// Lower-lid sag below the canthal line as a fraction of fissure width.
	curve: number;
};

export type PupillaryDistanceValue = {
	// Real pixels when `size` was passed to measure(); otherwise a fraction of
	// the image height (the mm figure is scale-free either way).
	px: number;
	mm: number;
	range: [number, number];
};
...
export type MetricValueMap = {
	"face-shape": FaceShapeValue;
	"face-symmetry": SymmetryValue;
	"golden-ratio": GoldenRatioValue;
	"canthal-tilt": CanthalTiltValue;
	"hunter-eyes": HunterEyesValue;
	"jawline-score": JawlineValue;
	"facial-ratios": FacialRatiosValue;
	"eye-shape": EyeShapeValue;
	"pupillary-distance": PupillaryDistanceValue;
};

export type MetricValue = MetricValueMap[ToolMetric];
...
const STOMION = 13;
const FOREHEAD_W: [number, number] = [54, 284];
const BIZYGOMATIC: [number, number] = [234, 454];
const BIGONIAL: [number, number] = [172, 397];
const CHIN_W: [number, number] = [176, 400];
const ALAE: [number, number] = [129, 358];
const MOUTH: [number, number] = [61, 291];
...
const EYE_METRICS: ReadonlySet<ToolMetric> = new Set<ToolMetric>([
	"canthal-tilt",
	"hunter-eyes",
	"eye-shape",
	"pupillary-distance",
]);
const IRIS_METRICS: ReadonlySet<ToolMetric> = new Set<ToolMetric>([
	"golden-ratio",
	"facial-ratios",
	"pupillary-distance",
]);
...
export function gateFor(
	...
	if (!pose) return { ok: false, reason: "points" };
	const box = faceBoxFromPoints(points);
	if (!box) return { ok: false, reason: "points" };

	const yawMax = EYE_METRICS.has(metric) ? YAW_EYES : YAW_STRICT;
	if (Math.abs(pose.yawRatio) > yawMax) return { ok: false, reason: "yaw" };
	if (pose.pitchRatio < PITCH_MIN || pose.pitchRatio > PITCH_MAX) {
		return { ok: false, reason: "pitch" };
	}
	if (Math.abs(pose.rollDeg) > ROLL_MAX) return { ok: false, reason: "roll" };
	if (box.h < MIN_FACE_HEIGHT) return { ok: false, reason: "small" };

	const pitchHigh =
		pose.pitchRatio >= PITCH_NEUTRAL - (PITCH_NEUTRAL - PITCH_MIN) / 2 &&
		pose.pitchRatio <= PITCH_NEUTRAL + (PITCH_MAX - PITCH_NEUTRAL) / 2;
	const high =
		Math.abs(pose.yawRatio) <= yawMax / 2 &&
...
function angleAt(at: Pt, a: Pt, b: Pt): number {
	const ax = a.x - at.x;
	const ay = a.y - at.y;
	const bx = b.x - at.x;
	const by = b.y - at.y;
...
function span(f: Pt[], [a, b]: [number, number]): number {
	return dist(f[a], f[b]);
}

// ——— Face shape ———
export type ShapePrototype = {
	shape: FaceShapeName;
	// [length/bizygomatic, forehead/bizygomatic, bigonial/bizygomatic,
	//  chin/bigonial, mean gonial angle in degrees]
	features: [number, number, number, number, number];
};

export const SHAPE_TOLERANCE: [number, number, number, number, number] = [
	0.12, 0.06, 0.06, 0.08, 10,
];

export const SHAPE_PROTOTYPES: readonly ShapePrototype[] = [
...
function gonialAngles(f: Pt[]): { right: number; left: number } {
	return {
...
export function faceShape(f: Pt[]): FaceShapeValue {
...
export const SYMMETRY_GROUPS: Record<
	keyof SymmetryValue["parts"],
	[number, number][]
> = {
	eyes: [
		[33, 263],
...
export const SYMMETRY_PAIRS: [number, number][] =
	Object.values(SYMMETRY_GROUPS).flat();
...
export function symmetry(f: Pt[]): SymmetryValue {
	const x0 = (f[FOREHEAD_TOP].x + f[MENTON].x) / 2;
	const width = span(f, BIZYGOMATIC);
	const part = (k: keyof SymmetryValue["parts"]) =>
		symmetryScore(f, x0, width, SYMMETRY_GROUPS[k]);
	return {
...
function ipd(f: Pt[]): number {
	return dist(f[EYE_R.pupil], f[EYE_L.pupil]);
}

export function goldenRatio(f: Pt[]): GoldenRatioValue {
...
function eyeStats(f: Pt[], e: Eye): EyeStats {
...
export function canthalTiltMetric(f: Pt[]): CanthalTiltValue {
	const right = eyeStats(f, EYE_R).tilt;
	const left = eyeStats(f, EYE_L).tilt;
	return { left, right, mean: (left + right) / 2 };
}

export function hunterEyes(f: Pt[]): HunterEyesValue {
	...
	const score =
		0.45 * ramp(ar, 0.42, 0.24) +
		0.35 * ramp(browGap, 0.75, 0.35) +
		0.2 * ramp(tilt, -2, 8);
	return { score, ar, browGap, tilt };
}

export function eyeShape(f: Pt[]): EyeShapeValue {
	const r = eyeStats(f, EYE_R);
	const l = eyeStats(f, EYE_L);
	const ar = (r.ar + l.ar) / 2;
	const tilt = (r.tilt + l.tilt) / 2;
...
// ——— Jawline ———
export function jawline(f: Pt[]): JawlineValue {
	const g = gonialAngles(f);
...
// ——— Facial ratios ———
export function facialRatios(f: Pt[]): FacialRatiosValue {
	...
	const fifths = [0, 1, 2, 3, 4].map(
		(i) => ((xs[i + 1] - xs[i]) / width) * 100,
...
export function pupillaryDistance(
	points: Landmark[],
	...
	const irisPx =
		rims.reduce((a, [p, q]) => a + dist(px(p), px(q)), 0) / rims.length;
	const mm = (ipdPx * IRIS_MM) / irisPx;
	return { px: ipdPx, mm, range: [mm * 0.93, mm * 1.07] };
}

// ——— Overlays ———
const EYE_LINES: [number, number][] = [
	[EYE_R.inner, EYE_R.outer],
	[EYE_L.inner, EYE_L.outer],
];

function overlayFor(metric: ToolMetric): Overlay {
	switch (metric) {
		case "face-shape":
			return {
				polylines: [JAW],
...
function compute<M extends ToolMetric>(
	metric: M,
	points: Landmark[],
	aspect: number,
	size: { w: number; h: number },
	...
	const table: { [K in ToolMetric]: () => MetricValueMap[K] } = {
 ...
 */
export function measure<M extends ToolMetric>(
	metric: M,
	points: Landmark[],
	aspect: number,
	size?: { w: number; h: number },
): MetricResult<MetricValueMap[M]> {
	...
	if (!gate.ok) return { gate, value: null, overlay };
	try {
